   ...
   class SwipeController(val context: Context, buttonsActions: SwipeControllerActions?) : Callback() {
    ...
    private fun setTouchListener(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float,
        ...
        recyclerView.setOnTouchListener { v, event ->
            ...
            if (swipeBack) {
                if (dX < -width) buttonShowedState = ButtonsState.RIGHT_VISIBLE
                else if (dX > width) buttonShowedState = ButtonsState.LEFT_VISIBLE
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun setTouchDownListener(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float,
                                     dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun setTouchUpListener(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float,
            dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
    ...
    private fun drawButtons(c: Canvas, viewHolder: RecyclerView.ViewHolder) {
        ...
        val leftButton = RectF(itemView.left.toFloat(), itemView.top.toFloat(),
                itemView.left.toFloat() + width, itemView.bottom.toFloat())

        p.color = ContextCompat.getColor(context, R.color.red)
        c.drawRect(leftButton, p)

        drawText(context.getString(R.string.reject_charity), c, leftButton, p)

        // left swipe
        val rightButton = RectF(itemView.right - width, itemView.top.toFloat(), itemView.right.toFloat(), itemView.bottom.toFloat())

        p.color = ContextCompat.getColor(context, R.color.colorPrimary)

        c.drawRect(rightButton, p)
        drawText(context.getString(R.string.accept_charity), c, rightButton, p)
